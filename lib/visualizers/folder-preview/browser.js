  ...
  fetch("/graph.json")
    ...
    .then(function (graph) {
      ...
      containers.forEach(function (container) {
        ...
        // Filter to pages in this folder (exclude current page, folder root, and index pages)
    ...
    const slides = pages
      .map(function (node) {
        const imgHtml = node.image
          ? `<img class="articles__image" src="${node.image}" alt="${node.title || ""}" loading="lazy">`
        ...
        return `<div class="swiper-slide articles__content">
          ...
          <a href="${node.id}" class="articles__read-more button-1">READ MORE</a>
        </div>`;
      })
      .join("");
    ...
    container.innerHTML = `
      <div class="articles__top-section">
        ...
        <div class="swiper-nav">
          <div class="swiper-button-prev articles__prev-button"></div>
          <div class="swiper-button-next articles__next-button"></div>
        </div>
      </div>
