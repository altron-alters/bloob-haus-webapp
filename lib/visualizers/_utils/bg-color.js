 ...
 *
 *   // No default — section is unstyled unless author sets bg=
 *   const { extraClass, style } = resolveBg(settings);
 *
 *   // With default — section always gets a color pair; author can override with bg=
 *   const { extraClass, style } = resolveBg(settings, "green");
 *
 ...
 * @param {string} [defaultBg]  Named token to use when author omits bg= (e.g. "green")
 ...
 */
export function resolveBg(settings, defaultBg = "") {
  const bg = (settings.bg || defaultBg).trim();
