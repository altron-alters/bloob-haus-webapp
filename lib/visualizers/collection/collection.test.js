...
describe("collection index.js exports", () => {
  ...
  it("transform preserves search setting in data-collection-settings", () => {
    // display:list never needs graph.json — always emits runtime placeholder,
    // so this test works without a real graph.json on disk.
    const basics = `<pre><code class="language-collection">source: folder=projects\ndisplay: list\nsearch: basics\n</code></pre>`;
    expect(mod.transform(basics)).toContain('"search":"basics"');

    const off = `<pre><code class="language-collection">source: folder=projects\ndisplay: list\nsearch: off\n</code></pre>`;
    expect(mod.transform(off)).toContain('"search":"off"');
  });
});
