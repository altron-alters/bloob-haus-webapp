...
(function () {
  ...
  function attachCombinedSearch(container, settings) {
    ...
    input.addEventListener("input", function () {
      ...
      cards.forEach(function (c) {
        ...
        if (hay.includes(norm)) metaMatches.add(c);
      });
      cards.forEach(function (c) { c.hidden = !metaMatches.has(c); });
        ...
        return pf.search(q, filters ? { filters: filters } : {}).then(function (results) {
          ...
          return Promise.all(results.results.map(function (r) { return r.data(); }))
            .then(function (data) {
              ...
              cards.forEach(function (c) {
                c.hidden = !metaMatches.has(c) && !pfUrls.has(c.getAttribute("href") || "");
              });
            });
        });
      });
  ...
  containers.forEach(function (container) {
    ...
    if (container.querySelector(".fp-seo-wrapper")) {
      if (settings.search === "basics") {
        attachSearch(container);
      } else {
        attachCombinedSearch(container, settings);
      }
    } else {
