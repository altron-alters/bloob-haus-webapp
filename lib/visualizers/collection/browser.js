...
(function () {
  ...
  function attachSearch(container) {
    ...
    input.addEventListener("input", function () {
      ...
      cards.forEach(function (card) {
        var haystack = (card.textContent + " " + (card.dataset.fpTags || "")).toLowerCase();
  ...
  containers.forEach(function (container) {
    // SEO mode: cards already in DOM — wire search and skip graph.json fetch
  ...
  function renderCards(container, pages, settings) {
    ...
    if (!searchDisabled) attachSearch(container);
  }
