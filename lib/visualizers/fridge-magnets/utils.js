 ...
 *   otherwise          → show if age < moderationHours
 */
export function passesModeration(row, moderationHours) {
  if (row.approved === "no") return false;
  if (row.approved === "yes") return true;
  const dateStr = row.Timestamp || row.submitted_at || "";
  if (!dateStr) return false;
  const ageHours = (Date.now() - new Date(dateStr).getTime()) / 3.6e6;
  return ageHours < moderationHours;
}
